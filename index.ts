...
async function call(path: string, body: unknown, config: any, signal?: AbortSignal) {
...
export default defineToolPlugin({
  ...
  name: "Blopus Search",
  description:
    "Web search and page fetch over an independently crawled index — not a reseller of " +
    "another engine's results. Freshness filtering, domain include/exclude, inline " +
    "content, and hero images.",
  configSchema: Type.Object({
    apiKey: Type.Optional(
      Type.String({ description: "Blopus API key (blp_live_...). Get one at https://blopus.ai" }),
    ),
    baseUrl: Type.Optional(
      Type.String({ description: `Blopus API base URL. Defaults to ${DEFAULT_BASE_URL}` }),
    ),
  }),
  tools: (tool) => [
    tool({
      ...
      label: "Blopus Search",
      description:
        "Search the web. Use news_only for event questions — what happened, who announced " +
      ...
      parameters: Type.Object({
        ...
        count: Type.Optional(
          Type.Number({ description: "Results to return; rounds up to a block of 10.", default: 10 }),
        ),
        ...
        news_only: Type.Optional(
          Type.Boolean({ description: "Restrict to news sources. Use for event questions." }),
        ),
        include_domains: Type.Optional(
          Type.Array(Type.String(), { description: "Only return results from these hostnames." }),
        ),
        exclude_domains: Type.Optional(
          Type.Array(Type.String(), { description: "Drop results from these hostnames." }),
        ),
        include_content: Type.Optional(
          Type.Boolean({ description: "Return full page text inline instead of a snippet." }),
        ),
      ...
      async execute(params, config, context) {
        ...
        const data: any = await call("/v1/search", params, config, context.signal);
        return {
          query: data.query,
          count: data.count,
    ...
    tool({
      ...
      label: "Blopus Fetch",
      description:
        "Fetch the stored full text of previously indexed URL(s) — use after a search when " +
      ...
      parameters: Type.Object({
        ...
        urls: Type.Optional(
          Type.Array(Type.String(), { description: "A batch of URLs (max 50). Prefer this." }),
        ),
      }),
      async execute(params, config, context) {
        context.signal?.throwIfAborted();
        ...
        const data: any = await call("/v1/fetch", params, config, context.signal);
        return data;
      },
    }),
  ],
